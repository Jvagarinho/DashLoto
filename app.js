...
function renderDrawnNumbers(numbers, stars) {
  ...
  numbersContainer.innerHTML = numbers
    .map((num) => `<div class="drawn-number">${num}</div>`)
    .join("");

  starsContainer.innerHTML = stars
    .map((star) => `<div class="drawn-star">${star}</div>`)
    .join("");
}
...
function checkTicket(userNumbers, userStars) {
  ...
  const matchedNumbers = currentDraw.numbers.filter((n) =>
    userNumbersSet.has(n),
  ).length;
  const matchedStars = currentDraw.stars.filter((s) =>
    userStarsSet.has(s),
  ).length;
  ...
  const userNumbersHtml = userNumbers
    .map((num, i) => {
...
function calculatePrize(matchedNumbers, matchedStars) {
  ...
  if (currentDraw.prizes && currentDraw.prizes.hasOwnProperty(prizeKey)) {
    ...
    if (typeof scrapedPrize === "number") {
      ...
      scrapedPrize !== null &&
      typeof scrapedPrize === "object" &&
      scrapedPrize.amount !== undefined
    ) {
      ...
      console.log(
        `⚠️ Prize ${prizeKey} found but has invalid/unparseable value:`,
        scrapedPrize,
      );
...
let fileHandle = null;
const FILE_NAME = "dashloto_favorites.json";

async function getFileHandle() {
  if (fileHandle) return fileHandle;

  // Try to get from session storage (persists during session)
  try {
    const storedHandle = sessionStorage.getItem("dashloto_file_handle");
    if (storedHandle) {
      fileHandle = await deserializeFileHandle(storedHandle);
      if (fileHandle) return fileHandle;
    }
  } catch (e) {
  ...
  // Fallback to localStorage if no file handle
  return null;
}
...
async function requestFileHandle() {
  try {
    ...
    sessionStorage.setItem(
      "dashloto_file_handle",
      await serializeFileHandle(handle),
    );
    return handle;
  } catch (e) {
...
async function serializeFileHandle(handle) {
  // FileSystemFileHandle doesn't serialize directly, store a reference
  // We'll just store a marker and re-prompt if needed
  return JSON.stringify({ name: handle.name, kind: handle.kind });
}

async function deserializeFileHandle(data) {
  // Cannot fully restore handle without user interaction
  // Return null to trigger re-prompt
  return null;
}

async function readFavoritesFromFile() {
  ...
  try {
    ...
    if (!data.euromilhoes) data.euromilhoes = [];
    if (!data.totoloto) data.totoloto = [];
    return data;
  } catch (e) {
    console.warn("Could not read favorites from file:", e);
    return null;
  }
}

async function writeFavoritesToFile(data) {
  ...
  try {
    ...
    console.error("Error writing favorites to file:", e);
    // If file was moved/deleted, request new handle
    if (e.name === "InvalidStateError" || e.name === "NotAllowedError") {
      fileHandle = null;
      sessionStorage.removeItem("dashloto_file_handle");
      return await writeFavoritesToFile(data); // Retry with new handle
    }
    return false;
  }
}
...
function getFavorites() {
  // First try to read from file (async), fall back to localStorage
  // For synchronous access, we use localStorage as cache
  try {
...
async function loadFavoritesFromFile() {
  const fileData = await readFavoritesFromFile();
  if (fileData) {
...
function renderFavorites() {
  ...
  container.innerHTML = gameFavorites
    .map((fav, idx) => {
      const numbersHtml = fav.numbers
        .map((n) => `<span class="fav-number">${n}</span>`)
        .join("");
      const starsHtml = fav.stars
        .map((s) => `<span class="fav-star">${s}</span>`)
        .join("");
      return `
            <div class="fav-item">
  ...
  container.querySelectorAll(".fav-btn-load").forEach((btn) => {
    btn.addEventListener("click", () =>
      loadFavorite(parseInt(btn.dataset.index)),
    );
  });
  container.querySelectorAll(".fav-btn-del").forEach((btn) => {
    btn.addEventListener("click", () =>
      deleteFavorite(parseInt(btn.dataset.index)),
    );
  });
}
...
async function verifyAllFavorites() {
  ...
  try {
    ...
    document.getElementById("draw-date").textContent =
      currentDraw.date.toLocaleDateString("pt-PT", dateOptions);
    ...
    multiContainer.innerHTML = gameFavorites
      .map((fav, idx) => {
        ...
        const isWin =
          (fallbackPrize &&
            currentDraw.prizes &&
            currentDraw.prizes[prizeKey] !== undefined) ||
          prizeKey === "0+1";
    ...
    const firstMatchedNumbers = currentDraw.numbers.filter((n) =>
      firstNumbersSet.has(n),
    ).length;
    const firstMatchedStars = currentDraw.stars.filter((s) =>
      firstStarsSet.has(s),
    ).length;

    document.getElementById("matched-numbers").textContent =
      firstMatchedNumbers;
    document.getElementById("matched-stars").textContent = firstMatchedStars;

    const userNumbersHtml = firstFav.numbers
      .map((n) => {
        const match = currentDraw.numbers.includes(n);
    ...
    const userStarsHtml = firstFav.stars
      .map((s) => {
        const match = currentDraw.stars.includes(s);
...
function switchGame(game) {
  ...
  for (let i = 0; i < config.starsCount; i++) {
    ...
    input.placeholder = "#";
    input.className =
      "star-input w-14 h-14 text-center text-lg border-2 border-gray-300 rounded-lg focus:border-santa-500 focus:ring-2 focus:ring-santa-200 outline-none";
    input.required = true;
...
document
  .getElementById("btn-euromilhoes")
  .addEventListener("click", () => switchGame("euromilhoes"));
document
  .getElementById("btn-totoloto")
  .addEventListener("click", () => switchGame("totoloto"));
...
document.getElementById("ticket-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const numbers = Array.from(
      document.querySelectorAll("#numbers-input input"),
    ).map((input) => parseInt(input.value));

    const stars = Array.from(
      document.querySelectorAll("#stars-input input"),
    ).map((input) => parseInt(input.value));
...
function importFavorites(file) {
  ...
  reader.onload = function (e) {
    try {
      ...
      alert(
        "Ficheiro inválido. Selecione um ficheiro .json gerado pela exportação.",
      );
    }
  };
...
document
  .getElementById("verify-all")
  .addEventListener("click", verifyAllFavorites);
document
...
document
  .getElementById("choose-save-location")
  .addEventListener("click", async () => {
    ...
    if (handle) {
      ...
      await writeFavoritesToFile(all);
      alert(
        "Local de gravação definido com sucesso! As chaves serão guardadas neste ficheiro.",
      );
    }
  });
